...
- `Sources/Sotto/SpeechEngine.swift`（重写）—— 录音 + 守护进程生命周期 + 转写
- `Sources/Sotto/AppDelegate.swift`—— 松手后显示 "Transcribing..."，去掉了苹果 Speech 的轮询计时器，只请求麦克风权限
- `Sources/Sotto/LLMRefiner.swift`—— 换成中文纠错提示词、加 `chat_template_kwargs`、放宽"必须有 API key"的限制
...
日志：`~/Library/Logs/Sotto.log`
重启：`osascript -e 'quit app "Sotto"'` 然后 `open Sotto.app`
